...
Поделим числитель и знаменатель правой части уравнения на $\wt$	и положим $y = \dfrac{\wt}{\wx}$. Получаем уравнение с разделяющимися переменными:
...
\begin{task4}
	...
	\[\dx = at^\alpha + bx^\beta\] \textbf{сводится} к линейному.
\end{task4}
%\begin{proof}[Подсказки к решению]
%
%\end{proof}
...
\begin{task5}
	...
	\begin{center}
		...
		\pic[draw=red, <->, angle eccentricity=2, angle radius=1.75cm]	{angle=M--t--o};
...
\begin{enumerate}
	\item Найти такую кривую $x(t)$, что для любой $t_0 \in \R$ касательная к $x(t)$ в точке $\big(t_0,\ x(t_0)\big)$ пересекает ось абсцисс в точке $\dfrac{t_0}{2}$.
