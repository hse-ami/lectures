...
%	Поймем, что это рекуррентное соотношение описывается своим деревом. Каждый $i$-ый уровень имеет сложность $5in$. Просуммируем по уровням, получим
%	\[
%	\sum_{i = 1}^{\log n} 5\cdot2^in = 5n \sum_ {i = 1}^{\log n} = \Theta(n).
%	\]
