...
\begin{Def}  
    Вписанной ломаной называется путь, состоящий их отрезков $[t_{i-1}, t_i]$ \\($i = 1\ldots n$), где $t_i = \varphi(x_i)$.
\end{Def}
\usetikzlibrary{calc}
\begin{center}
    \begin{tikzpicture}
    ...
    \draw[scale=1, domain=0:8.0, smooth,variable=\x, red] plot ({\x},{-0.0666667*\x*\x*\x + 0.6666667*\x*\x -1.066667*\x + 2});
...
Пусть отображение $\varphi \colon [a; b] \to \R^n$ --- кусочно-гладкий путь, $\gamma = \varphi([a;b])$ --- кривая, и есть некоторая функция $f\colon \gamma \to \R$.
\begin{Def}
