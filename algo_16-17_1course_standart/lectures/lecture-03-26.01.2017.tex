...
\begin{document}
	...
	\begin{center}
		\begin{tikzpicture}
			\matrix[matrix of math nodes, row sep=2mm] (M) {
				& & A_1 & & & & A_2 &  \\
				\leftbracket & A[i_0] & \ldots & A[i_{k}] &  p  & A[i_{k + 1}] & \ldots & A[i_{n - 1}] &  \rightbracket \\
				& & \leqslant p & & & & \geqslant p &  \\
			};
		\end{tikzpicture}
	\end{center}
	...
	\begin{center}
		\begin{tikzpicture}
		\matrix[matrix of math nodes, row sep=2mm] (M) {
			\leftbracket A[0] & \ldots & A[l] & & & \ldots &\ldots & & & A[r]& \ldots& A[n -1] \rightbracket \\
			\leftbracket A[0] & \ldots & A[i_0] & \ldots & A[i_{\frac{k}{2}}] & A[l] & A[i_{{\frac{k}{2}}+1}] & \ldots & A[i_k] & A[r]& \ldots& A[n - 1] \rightbracket \\
		};
		\end{tikzpicture}
	\end{center}
	...
	\begin{time}
		...
		%\TODO{Написать о j подзадаче}
		\begin{definition}
			\textit{j-подзадача} - это подзадача размера $s$ такая, что
			\[
			\exists j: \ \left(\frac{3}{4}\right)^{j - 1} \leqslant s \leqslant \left(\frac{3}{4}\right)^{j}
			\]
		\end{definition}
		На $j$ подзадачу тратится время $O((\frac{3}{4})^jn)$, на все $j$ подзадач - $O((\frac{4}{3})^{j + 1})$. В итоге получаем, что среднее время на все j-подзадачи
		\[
		T(n) = 	O\left(
					\left(
						\frac{3}{4}
